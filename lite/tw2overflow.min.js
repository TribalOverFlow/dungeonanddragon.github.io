...
;var Fe=[50,30,60,80,125,250,200,300,320,0,4e4,4e3,1200],qe=[30,30,30,30,100,100,150,200,400,0,5e4,2e3,1200],Ye=[20,70,40,60,250,150,600,200,100,0,5e4,2e3,2400],He=[1,1,1,1,4,5,6,5,8,1,100,10,6],je=0,Ke=0,Qe=0,Xe=0,Ze=0,Je=0,$e=0,et=0,tt=0,at=0,nt=0,ot=0,it=0,lt='',st=0,rt=0,ct=0,dt=0,ut=0,pt=0,gt=0,mt=0,Tt=0,ft=0,_t=0,bt=0,ht=0,At=0,Et=0,wt=0,vt=0,St=0,Rt=0,yt=0,kt=0,Mt=0,It=0,Nt=0,xt=0,Ot=0,Lt=0,Pt=0,Ut=0,Ct=0,zt=0,Dt=0,Gt=0,Bt=0,Vt=0,Wt=0,Ft=0,qt=0,Yt=0,Ht=0,jt=0,Kt=0,Qt=0,Xt=0,Zt=0,Jt=0,$t=0,ea=0,ta=0,aa=0,na=0,oa=0,ia=0,la=0,sa=0,ra=0,ca=0,da=0,ua=0,pa=[],ga=0,ma=0,Ta=0,fa=[],_a=0,ba=0,ha=0,Aa=[],Ea=0,wa=0,va=0,Sa=[],Ra=0,ya=0,ka=0,Ma=0,Ia=0,Na=0,xa=0,Oa=0,La=0,Pa=0,Ua=0,Ca=0,za=0,Da=0,Ga=0,Ba=0,Va=0,Wa=0,Fa=0,qa=0,Ya=0,Ha=0,ja=0,Ka=0,Qa=0,Xa=0,Za=0,Ja=0,$a=0,en=0,tn=0,an=0,nn=0;function on(){ma=t>0?nt.length>=2?1:1==nt.length?.5:.25:0,function(){ga=pa.length>0?pa.reduce((function(e,t){return e+t})):0;!function(){
Ta=t>0?ga>=936e5?0:ga<936e5&&ga>=864e5?.1:ga<864e5&&ga>=72e6?.25:ga<72e6&&ga>=576e5?.4:ga<576e5&&ga>=432e5?.55:ga<432e5&&ga>=288e5?.7:ga<288e5&&ga>=144e5?.85:ga<144e5&&ga>=72e5?.925:ga<72e5&&ga>=36e5?.9625:1:0;e=me-je,a=Te-Ke,n=re-Qe,o=se-Xe,r=ue-Ze,c=pe-Je,d=ge-$e,u=ce-et,p=de-tt,g=Math.floor(x*Ta*ma),T=Math.floor(O*Ta*ma),f=Math.floor(R*Ta*ma),b=Math.floor(S*Ta*ma),h=Math.floor(M*Ta*ma),A=Math.floor(I*Ta*ma),E=Math.floor(N*Ta*ma),w=Math.floor(y*Ta*ma),L=Math.floor(k*Ta*ma),console.log(g,T,f,b,h,A,E,L,w),Ma=g*Fe[0]+T*Fe[1]+f*Fe[2]+b*Fe[3]+h*Fe[4]+A*Fe[5]+E*Fe[7]+w*Fe[8]+L*Fe[6],Ia=g*qe[0]+T*qe[1]+f*qe[2]+b*qe[3]+h*qe[4]+A*qe[5]+E*qe[7]+w*qe[8]+L*qe[6],xa=g*Ye[0]+T*Ye[1]+f*Ye[2]+b*Ye[3]+h*Ye[4]+A*Ye[5]+E*Ye[7]+w*Ye[8]+L*Ye[6],Na=g*He[0]+T*He[1]+f*He[2]+b*He[3]+h*He[4]+A*He[5]+E*He[7]+w*He[8]+L*He[6],console.log(Ma,Ia,xa,Na),
e<=0||p<=0||u<=0||a<=0||n<=0||o<=0||r<=0||c<=0||d<=0?console.log('Brak jednostek do rekrutacji'):(Ma<=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia<=Pa&&xa<=Ua&&Na<=Oa?(na=g,sa=T,Tt=f,ut=b,Ft=h,jt=A,Zt=E,ht=w,Ut=L):Ma>=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia<=Pa&&xa<=Ua&&Na<=Oa?(xt=La/Ma,na=Math.floor(xt*g)-1,sa=Math.floor(xt*T)-1,Tt=Math.floor(xt*f)-1,ut=Math.floor(xt*b)-1,Ft=Math.floor(xt*h)-1,jt=Math.floor(xt*A)-1,Zt=Math.floor(xt*E)-1,ht=Math.floor(xt*w)-1,Ut=Math.floor(xt*L)-1):Ma<=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia>=Pa&&xa<=Ua&&Na<=Oa?(xt=Pa/Ia,na=Math.floor(xt*g)-1,sa=Math.floor(xt*T)-1,Tt=Math.floor(xt*f)-1,ut=Math.floor(xt*b)-1,Ft=Math.floor(xt*h)-1,jt=Math.floor(xt*A)-1,Zt=Math.floor(xt*E)-1,ht=Math.floor(xt*w)-1,Ut=Math.floor(xt*L)-1):Ma<=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia<=Pa&&xa>=Ua&&Na<=Oa?(xt=Ua/xa,na=Math.floor(xt*g)-1,sa=Math.floor(xt*T)-1,Tt=Math.floor(xt*f)-1,
ut=Math.floor(xt*b)-1,Ft=Math.floor(xt*h)-1,jt=Math.floor(xt*A)-1,Zt=Math.floor(xt*E)-1,ht=Math.floor(xt*w)-1,Ut=Math.floor(xt*L)-1):Ma<=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia<=Pa&&xa<=Ua&&Na>=Oa?(xt=Oa/Na,na=Math.floor(xt*g)-1,sa=Math.floor(xt*T)-1,Tt=Math.floor(xt*f)-1,ut=Math.floor(xt*b)-1,Ft=Math.floor(xt*h)-1,jt=Math.floor(xt*A)-1,Zt=Math.floor(xt*E)-1,ht=Math.floor(xt*w)-1,Ut=Math.floor(xt*L)-1):Ma>=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia>=Pa&&xa<=Ua&&Na<=Oa?(vt=Pa/Ia)>(st=La/Ma)?(na=Math.floor(st*g)-1,sa=Math.floor(st*T)-1,Tt=Math.floor(st*f)-1,ut=Math.floor(st*b)-1,Ft=Math.floor(st*h)-1,jt=Math.floor(st*A)-1,Zt=Math.floor(st*E)-1,ht=Math.floor(st*w)-1,Ut=Math.floor(st*L)-1):(na=Math.floor(vt*g)-1,sa=Math.floor(vt*T)-1,Tt=Math.floor(vt*f)-1,ut=Math.floor(vt*b)-1,Ft=Math.floor(vt*h)-1,jt=Math.floor(vt*A)-1,Zt=Math.floor(vt*E)-1,ht=Math.floor(vt*w)-1,
Ut=Math.floor(vt*L)-1):Ma<=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia>=Pa&&xa>=Ua&&Na<=Oa?(vt=Pa/Ia)>(Gt=Ua/xa)?(na=Math.floor(Gt*g)-1,sa=Math.floor(Gt*T)-1,Tt=Math.floor(Gt*f)-1,ut=Math.floor(Gt*b)-1,Ft=Math.floor(Gt*h)-1,jt=Math.floor(Gt*A)-1,Zt=Math.floor(Gt*E)-1,ht=Math.floor(Gt*w)-1,Ut=Math.floor(Gt*L)-1):(na=Math.floor(vt*g)-1,sa=Math.floor(vt*T)-1,Tt=Math.floor(vt*f)-1,ut=Math.floor(vt*b)-1,Ft=Math.floor(vt*h)-1,jt=Math.floor(vt*A)-1,Zt=Math.floor(vt*E)-1,ht=Math.floor(vt*w)-1,Ut=Math.floor(vt*L)-1):Ma<=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia>=Pa&&xa<=Ua&&Na>=Oa?(vt=Pa/Ia)>(kt=Oa/Na)?(na=Math.floor(kt*g)-1,sa=Math.floor(kt*T)-1,Tt=Math.floor(kt*f)-1,ut=Math.floor(kt*b)-1,Ft=Math.floor(kt*h)-1,jt=Math.floor(kt*A)-1,Zt=Math.floor(kt*E)-1,ht=Math.floor(kt*w)-1,Ut=Math.floor(kt*L)-1):(na=Math.floor(vt*g)-1,sa=Math.floor(vt*T)-1,Tt=Math.floor(vt*f)-1,ut=Math.floor(vt*b)-1,Ft=Math.floor(vt*h)-1,jt=Math.floor(vt*A)-1,Zt=Math.floor(vt*E)-1,
ht=Math.floor(vt*w)-1,Ut=Math.floor(vt*L)-1):Ma>=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia<=Pa&&xa>=Ua&&Na<=Oa?(Gt=Ua/xa)>(st=La/Ma)?(na=Math.floor(st*g)-1,sa=Math.floor(st*T)-1,Tt=Math.floor(st*f)-1,ut=Math.floor(st*b)-1,Ft=Math.floor(st*h)-1,jt=Math.floor(st*A)-1,Zt=Math.floor(st*E)-1,ht=Math.floor(st*w)-1,Ut=Math.floor(st*L)-1):(na=Math.floor(Gt*g)-1,sa=Math.floor(Gt*T)-1,Tt=Math.floor(Gt*f)-1,ut=Math.floor(Gt*b)-1,Ft=Math.floor(Gt*h)-1,jt=Math.floor(Gt*A)-1,Zt=Math.floor(Gt*E)-1,ht=Math.floor(Gt*w)-1,Ut=Math.floor(Gt*L)-1):Ma<=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia<=Pa&&xa>=Ua&&Na>=Oa?(Gt=Ua/xa)>(kt=Oa/Na)?(na=Math.floor(kt*g)-1,sa=Math.floor(kt*T)-1,Tt=Math.floor(kt*f)-1,ut=Math.floor(kt*b)-1,Ft=Math.floor(kt*h)-1,jt=Math.floor(kt*A)-1,Zt=Math.floor(kt*E)-1,ht=Math.floor(kt*w)-1,Ut=Math.floor(kt*L)-1):(na=Math.floor(Gt*g)-1,sa=Math.floor(Gt*T)-1,Tt=Math.floor(Gt*f)-1,ut=Math.floor(Gt*b)-1,Ft=Math.floor(Gt*h)-1,jt=Math.floor(Gt*A)-1,
Zt=Math.floor(Gt*E)-1,ht=Math.floor(Gt*w)-1,Ut=Math.floor(Gt*L)-1):Ma>=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia<=Pa&&xa<=Ua&&Na>=Oa?(kt=Oa/Na)>(st=La/Ma)?(na=Math.floor(st*g)-1,sa=Math.floor(st*T)-1,Tt=Math.floor(st*f)-1,ut=Math.floor(st*b)-1,Ft=Math.floor(st*h)-1,jt=Math.floor(st*A)-1,Zt=Math.floor(st*E)-1,ht=Math.floor(st*w)-1,Ut=Math.floor(st*L)-1):(na=Math.floor(kt*g)-1,sa=Math.floor(kt*T)-1,Tt=Math.floor(kt*f)-1,ut=Math.floor(kt*b)-1,Ft=Math.floor(kt*h)-1,jt=Math.floor(kt*A)-1,Zt=Math.floor(kt*E)-1,ht=Math.floor(kt*w)-1,Ut=Math.floor(kt*L)-1):Ma>=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia>=Pa&&xa>=Ua&&Na<=Oa?(st=La/Ma,(vt=Pa/Ia)>(Gt=Ua/xa)&&st>Gt?(na=Math.floor(Gt*g)-1,sa=Math.floor(Gt*T)-1,Tt=Math.floor(Gt*f)-1,ut=Math.floor(Gt*b)-1,Ft=Math.floor(Gt*h)-1,jt=Math.floor(Gt*A)-1,Zt=Math.floor(Gt*E)-1,ht=Math.floor(Gt*w)-1,Ut=Math.floor(Gt*L)-1):Gt>vt&&st>vt?(na=Math.floor(vt*g)-1,sa=Math.floor(vt*T)-1,Tt=Math.floor(vt*f)-1,
ut=Math.floor(vt*b)-1,Ft=Math.floor(vt*h)-1,jt=Math.floor(vt*A)-1,Zt=Math.floor(vt*E)-1,ht=Math.floor(vt*w)-1,Ut=Math.floor(vt*L)-1):Gt>st&&vt>st?(na=Math.floor(st*g)-1,sa=Math.floor(st*T)-1,Tt=Math.floor(st*f)-1,ut=Math.floor(st*b)-1,Ft=Math.floor(st*h)-1,jt=Math.floor(st*A)-1,Zt=Math.floor(st*E)-1,ht=Math.floor(st*w)-1,Ut=Math.floor(st*L)-1):console.log('Nie znam przyczyny :P')):Ma>=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia>=Pa&&xa<=Ua&&Na>=Oa?(st=La/Ma,(vt=Pa/Ia)>(kt=Oa/Na)&&st>kt?(na=Math.floor(kt*g)-1,sa=Math.floor(kt*T)-1,Tt=Math.floor(kt*f)-1,ut=Math.floor(kt*b)-1,Ft=Math.floor(kt*h)-1,jt=Math.floor(kt*A)-1,Zt=Math.floor(kt*E)-1,ht=Math.floor(kt*w)-1,Ut=Math.floor(kt*L)-1):kt>vt&&st>vt?(na=Math.floor(vt*g)-1,sa=Math.floor(vt*T)-1,Tt=Math.floor(vt*f)-1,ut=Math.floor(vt*b)-1,Ft=Math.floor(vt*h)-1,jt=Math.floor(vt*A)-1,Zt=Math.floor(vt*E)-1,ht=Math.floor(vt*w)-1,Ut=Math.floor(vt*L)-1):kt>st&&vt>st?(na=Math.floor(st*g)-1,sa=Math.floor(st*T)-1,
Tt=Math.floor(st*f)-1,ut=Math.floor(st*b)-1,Ft=Math.floor(st*h)-1,jt=Math.floor(st*A)-1,Zt=Math.floor(st*E)-1,ht=Math.floor(st*w)-1,Ut=Math.floor(st*L)-1):console.log('Nie znam przyczyny :P')):Ma>=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia<=Pa&&xa>=Ua&&Na>=Oa?(st=La/Ma,(kt=Oa/Na)>(Gt=Ua/xa)&&st>Gt?(na=Math.floor(Gt*g)-1,sa=Math.floor(Gt*T)-1,Tt=Math.floor(Gt*f)-1,ut=Math.floor(Gt*b)-1,Ft=Math.floor(Gt*h)-1,jt=Math.floor(Gt*A)-1,Zt=Math.floor(Gt*E)-1,ht=Math.floor(Gt*w)-1,Ut=Math.floor(Gt*L)-1):Gt>kt&&st>kt?(na=Math.floor(kt*g)-1,sa=Math.floor(kt*T)-1,Tt=Math.floor(kt*f)-1,ut=Math.floor(kt*b)-1,Ft=Math.floor(kt*h)-1,jt=Math.floor(kt*A)-1,Zt=Math.floor(kt*E)-1,ht=Math.floor(kt*w)-1,Ut=Math.floor(kt*L)-1):Gt>st&&kt>st?(na=Math.floor(st*g)-1,sa=Math.floor(st*T)-1,Tt=Math.floor(st*f)-1,ut=Math.floor(st*b)-1,Ft=Math.floor(st*h)-1,jt=Math.floor(st*A)-1,Zt=Math.floor(st*E)-1,ht=Math.floor(st*w)-1,
Ut=Math.floor(st*L)-1):console.log('Nie znam przyczyny :P')):Ma<=La&&(e>=_||p>=_||u>=_||a>=_||n>=_||o>=_||r>=_||c>=_||d>=_)&&Ia>=Pa&&xa>=Ua&&Na>=Oa&&(kt=Oa/Na,(vt=Pa/Ia)>(Gt=Ua/xa)&&kt>Gt?(na=Math.floor(Gt*g)-1,sa=Math.floor(Gt*T)-1,Tt=Math.floor(Gt*f)-1,ut=Math.floor(Gt*b)-1,Ft=Math.floor(Gt*h)-1,jt=Math.floor(Gt*A)-1,Zt=Math.floor(Gt*E)-1,ht=Math.floor(Gt*w)-1,Ut=Math.floor(Gt*L)-1):Gt>vt&&kt>vt?(na=Math.floor(vt*g)-1,sa=Math.floor(vt*T)-1,Tt=Math.floor(vt*f)-1,ut=Math.floor(vt*b)-1,Ft=Math.floor(vt*h)-1,jt=Math.floor(vt*A)-1,Zt=Math.floor(vt*E)-1,ht=Math.floor(vt*w)-1,Ut=Math.floor(vt*L)-1):Gt>kt&&vt>kt?(na=Math.floor(kt*g)-1,sa=Math.floor(kt*T)-1,Tt=Math.floor(kt*f)-1,ut=Math.floor(kt*b)-1,Ft=Math.floor(kt*h)-1,jt=Math.floor(kt*A)-1,Zt=Math.floor(kt*E)-1,ht=Math.floor(kt*w)-1,Ut=Math.floor(kt*L)-1):console.log('Nie znam przyczyny :P')),setTimeout((function(){na>=_?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'spear',amount:na}),v=[lt='spear',na],
m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,na)):console.log('Niezrekrutowano pikinierów - mniej niż wskazane minimum lub zbyt niski poziom koszar')}),6e3),setTimeout((function(){at>=3&&(sa>=_?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'sword',amount:sa}),v=[lt='sword',sa],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,sa)):console.log('Niezrekrutowano mieczników - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),12e3),setTimeout((function(){at>=5&&(Tt>=_?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'axe',amount:Tt}),v=[lt='axe',Tt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,Tt)):console.log('Niezrekrutowano toporników - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),18e3),setTimeout((function(){at>=9&&(ut>=_?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'archer',amount:ut}),v=[lt='archer',ut],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),
J(Ca,lt,ut)):console.log('Niezrekrutowano łuczników - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),24e3),setTimeout((function(){at>=11&&(Ft>=_?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'light_cavalry',amount:Ft}),v=[lt='light_cavalry',Ft],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,Ft)):console.log('Niezrekrutowano lk - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),3e4),setTimeout((function(){at>=13&&(jt>=_?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'mounted_archer',amount:jt}),v=[lt='mounted_archer',jt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,jt)):console.log('Niezrekrutowano łk - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),36e3),setTimeout((function(){at>=15&&(Zt>=_?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'ram',amount:Zt}),v=[lt='ram',Zt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,Zt)):console.log('Niezrekrutowano taranów - mniej niż wskazane minimum lub zbyt niski poziom koszar'))
}),42e3),setTimeout((function(){at>=17&&(ht>=_?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'catapult',amount:ht}),v=[lt='catapult',ht],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,ht)):console.log('Niezrekrutowano katapult - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),48e3),setTimeout((function(){at>=21&&(Ut>=_?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'heavy_cavalry',amount:Ut}),v=[lt='heavy_cavalry',Ut],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,Ut)):console.log('Niezrekrutowano ck - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),54e3));var e,a,n,o,r,c,d,u,p,g,T,f,b,h,A,E,w,L}()}()}function ln(){ba=a>0?Da.length>=2?1:1==Da.length?.5:.25:0,function(){_a=fa.length>0?fa.reduce((function(e,t){return e+t})):0;!function(){ha=a>0?_a>=936e5?0:_a<936e5&&_a>=864e5?.1:_a<864e5&&_a>=72e6?.25:_a<72e6&&_a>=576e5?.4:_a<576e5&&_a>=432e5?.55:_a<432e5&&_a>=288e5?.7:_a<288e5&&_a>=144e5?.85:_a<144e5&&_a>=72e5?.925:_a<72e5&&_a>=36e5?.9625:1:0
;e=ve-je,t=Se-Ke,n=_e-Qe,o=fe-Xe,r=Ae-Ze,c=Ee-Je,d=we-$e,u=be-et,p=he-tt,g=Math.floor(B*ha*ba),T=Math.floor(V*ha*ba),f=Math.floor(P*ha*ba),_=Math.floor(L*ha*ba),b=Math.floor(z*ha*ba),A=Math.floor(D*ha*ba),E=Math.floor(G*ha*ba),w=Math.floor(U*ha*ba),S=Math.floor(C*ha*ba),R=g*Fe[0]+T*Fe[1]+f*Fe[2]+_*Fe[3]+b*Fe[4]+A*Fe[5]+E*Fe[7]+w*Fe[8]+S*Fe[6],y=g*qe[0]+T*qe[1]+f*qe[2]+_*qe[3]+b*qe[4]+A*qe[5]+E*qe[7]+w*qe[8]+S*qe[6],k=g*Ye[0]+T*Ye[1]+f*Ye[2]+_*Ye[3]+b*Ye[4]+A*Ye[5]+E*Ye[7]+w*Ye[8]+S*Ye[6],M=g*He[0]+T*He[1]+f*He[2]+_*He[3]+b*He[4]+A*He[5]+E*He[7]+w*He[8]+S*He[6],console.log(R,y,k,M),e<=0||p<=0||u<=0||t<=0||n<=0||o<=0||r<=0||c<=0||d<=0?console.log('Brak jednostek do rekrutacji'):(R<=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y<=Wa&&k<=Fa&&M<=Ba?(oa=g,ra=T,ft=f,pt=_,qt=b,Kt=A,Jt=E,At=w,Ct=S):R>=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y<=Wa&&k<=Fa&&M<=Ba?(Ot=Va/R,oa=Math.floor(Ot*g)-1,ra=Math.floor(Ot*T)-1,ft=Math.floor(Ot*f)-1,pt=Math.floor(Ot*_)-1,
qt=Math.floor(Ot*b)-1,Kt=Math.floor(Ot*A)-1,Jt=Math.floor(Ot*E)-1,At=Math.floor(Ot*w)-1,Ct=Math.floor(Ot*S)-1):R<=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y>=Wa&&k<=Fa&&M<=Ba?(Ot=Wa/y,oa=Math.floor(Ot*g)-1,ra=Math.floor(Ot*T)-1,ft=Math.floor(Ot*f)-1,pt=Math.floor(Ot*_)-1,qt=Math.floor(Ot*b)-1,Kt=Math.floor(Ot*A)-1,Jt=Math.floor(Ot*E)-1,At=Math.floor(Ot*w)-1,Ct=Math.floor(Ot*S)-1):R<=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y<=Wa&&k>=Fa&&M<=Ba?(Ot=Fa/k,oa=Math.floor(Ot*g)-1,ra=Math.floor(Ot*T)-1,ft=Math.floor(Ot*f)-1,pt=Math.floor(Ot*_)-1,qt=Math.floor(Ot*b)-1,Kt=Math.floor(Ot*A)-1,Jt=Math.floor(Ot*E)-1,At=Math.floor(Ot*w)-1,Ct=Math.floor(Ot*S)-1):R<=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y<=Wa&&k<=Fa&&M>=Ba?(Ot=Ba/M,oa=Math.floor(Ot*g)-1,ra=Math.floor(Ot*T)-1,ft=Math.floor(Ot*f)-1,pt=Math.floor(Ot*_)-1,qt=Math.floor(Ot*b)-1,Kt=Math.floor(Ot*A)-1,Jt=Math.floor(Ot*E)-1,At=Math.floor(Ot*w)-1,
Ct=Math.floor(Ot*S)-1):R>=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y>=Wa&&k<=Fa&&M<=Ba?(St=Wa/y)>(rt=Va/R)?(oa=Math.floor(rt*g)-1,ra=Math.floor(rt*T)-1,ft=Math.floor(rt*f)-1,pt=Math.floor(rt*_)-1,qt=Math.floor(rt*b)-1,Kt=Math.floor(rt*A)-1,Jt=Math.floor(rt*E)-1,At=Math.floor(rt*w)-1,Ct=Math.floor(rt*S)-1):(oa=Math.floor(St*g)-1,ra=Math.floor(St*T)-1,ft=Math.floor(St*f)-1,pt=Math.floor(St*_)-1,qt=Math.floor(St*b)-1,Kt=Math.floor(St*A)-1,Jt=Math.floor(St*E)-1,At=Math.floor(St*w)-1,Ct=Math.floor(St*S)-1):R<=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y>=Wa&&k>=Fa&&M<=Ba?(St=Wa/y)>(Bt=Fa/k)?(oa=Math.floor(Bt*g)-1,ra=Math.floor(Bt*T)-1,ft=Math.floor(Bt*f)-1,pt=Math.floor(Bt*_)-1,qt=Math.floor(Bt*b)-1,Kt=Math.floor(Bt*A)-1,Jt=Math.floor(Bt*E)-1,At=Math.floor(Bt*w)-1,Ct=Math.floor(Bt*S)-1):(oa=Math.floor(St*g)-1,ra=Math.floor(St*T)-1,ft=Math.floor(St*f)-1,pt=Math.floor(St*_)-1,qt=Math.floor(St*b)-1,Kt=Math.floor(St*A)-1,Jt=Math.floor(St*E)-1,
At=Math.floor(St*w)-1,Ct=Math.floor(St*S)-1):R<=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y>=Wa&&k<=Fa&&M>=Ba?(St=Wa/y)>(Mt=Ba/M)?(oa=Math.floor(Mt*g)-1,ra=Math.floor(Mt*T)-1,ft=Math.floor(Mt*f)-1,pt=Math.floor(Mt*_)-1,qt=Math.floor(Mt*b)-1,Kt=Math.floor(Mt*A)-1,Jt=Math.floor(Mt*E)-1,At=Math.floor(Mt*w)-1,Ct=Math.floor(Mt*S)-1):(oa=Math.floor(St*g)-1,ra=Math.floor(St*T)-1,ft=Math.floor(St*f)-1,pt=Math.floor(St*_)-1,qt=Math.floor(St*b)-1,Kt=Math.floor(St*A)-1,Jt=Math.floor(St*E)-1,At=Math.floor(St*w)-1,Ct=Math.floor(St*S)-1):R>=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y<=Wa&&k>=Fa&&M<=Ba?(Bt=Fa/k)>(rt=Va/R)?(oa=Math.floor(rt*g)-1,ra=Math.floor(rt*T)-1,ft=Math.floor(rt*f)-1,pt=Math.floor(rt*_)-1,qt=Math.floor(rt*b)-1,Kt=Math.floor(rt*A)-1,Jt=Math.floor(rt*E)-1,At=Math.floor(rt*w)-1,Ct=Math.floor(rt*S)-1):(oa=Math.floor(Bt*g)-1,ra=Math.floor(Bt*T)-1,ft=Math.floor(Bt*f)-1,pt=Math.floor(Bt*_)-1,qt=Math.floor(Bt*b)-1,Kt=Math.floor(Bt*A)-1,
Jt=Math.floor(Bt*E)-1,At=Math.floor(Bt*w)-1,Ct=Math.floor(Bt*S)-1):R<=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y<=Wa&&k>=Fa&&M>=Ba?(Bt=Fa/k)>(Mt=Ba/M)?(oa=Math.floor(Mt*g)-1,ra=Math.floor(Mt*T)-1,ft=Math.floor(Mt*f)-1,pt=Math.floor(Mt*_)-1,qt=Math.floor(Mt*b)-1,Kt=Math.floor(Mt*A)-1,Jt=Math.floor(Mt*E)-1,At=Math.floor(Mt*w)-1,Ct=Math.floor(Mt*S)-1):(oa=Math.floor(Bt*g)-1,ra=Math.floor(Bt*T)-1,ft=Math.floor(Bt*f)-1,pt=Math.floor(Bt*_)-1,qt=Math.floor(Bt*b)-1,Kt=Math.floor(Bt*A)-1,Jt=Math.floor(Bt*E)-1,At=Math.floor(Bt*w)-1,Ct=Math.floor(Bt*S)-1):R>=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y<=Wa&&k<=Fa&&M>=Ba?(Mt=Ba/M)>(rt=Va/R)?(oa=Math.floor(rt*g)-1,ra=Math.floor(rt*T)-1,ft=Math.floor(rt*f)-1,pt=Math.floor(rt*_)-1,qt=Math.floor(rt*b)-1,Kt=Math.floor(rt*A)-1,Jt=Math.floor(rt*E)-1,At=Math.floor(rt*w)-1,Ct=Math.floor(rt*S)-1):(oa=Math.floor(Mt*g)-1,ra=Math.floor(Mt*T)-1,ft=Math.floor(Mt*f)-1,pt=Math.floor(Mt*_)-1,qt=Math.floor(Mt*b)-1,
Kt=Math.floor(Mt*A)-1,Jt=Math.floor(Mt*E)-1,At=Math.floor(Mt*w)-1,Ct=Math.floor(Mt*S)-1):R>=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y>=Wa&&k>=Fa&&M<=Ba?(rt=Va/R,(St=Wa/y)>(Bt=Fa/k)&&rt>Bt?(oa=Math.floor(Bt*g)-1,ra=Math.floor(Bt*T)-1,ft=Math.floor(Bt*f)-1,pt=Math.floor(Bt*_)-1,qt=Math.floor(Bt*b)-1,Kt=Math.floor(Bt*A)-1,Jt=Math.floor(Bt*E)-1,At=Math.floor(Bt*w)-1,Ct=Math.floor(Bt*S)-1):Bt>St&&rt>St?(oa=Math.floor(St*g)-1,ra=Math.floor(St*T)-1,ft=Math.floor(St*f)-1,pt=Math.floor(St*_)-1,qt=Math.floor(St*b)-1,Kt=Math.floor(St*A)-1,Jt=Math.floor(St*E)-1,At=Math.floor(St*w)-1,Ct=Math.floor(St*S)-1):Bt>rt&&St>rt?(oa=Math.floor(rt*g)-1,ra=Math.floor(rt*T)-1,ft=Math.floor(rt*f)-1,pt=Math.floor(rt*_)-1,qt=Math.floor(rt*b)-1,Kt=Math.floor(rt*A)-1,Jt=Math.floor(rt*E)-1,At=Math.floor(rt*w)-1,Ct=Math.floor(rt*S)-1):console.log('Nie znam przyczyny :P')):R>=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y>=Wa&&k<=Fa&&M>=Ba?(rt=Va/R,
(St=Wa/y)>(Mt=Ba/M)&&rt>Mt?(oa=Math.floor(Mt*g)-1,ra=Math.floor(Mt*T)-1,ft=Math.floor(Mt*f)-1,pt=Math.floor(Mt*_)-1,qt=Math.floor(Mt*b)-1,Kt=Math.floor(Mt*A)-1,Jt=Math.floor(Mt*E)-1,At=Math.floor(Mt*w)-1,Ct=Math.floor(Mt*S)-1):Mt>St&&rt>St?(oa=Math.floor(St*g)-1,ra=Math.floor(St*T)-1,ft=Math.floor(St*f)-1,pt=Math.floor(St*_)-1,qt=Math.floor(St*b)-1,Kt=Math.floor(St*A)-1,Jt=Math.floor(St*E)-1,At=Math.floor(St*w)-1,Ct=Math.floor(St*S)-1):Mt>rt&&St>rt?(oa=Math.floor(rt*g)-1,ra=Math.floor(rt*T)-1,ft=Math.floor(rt*f)-1,pt=Math.floor(rt*_)-1,qt=Math.floor(rt*b)-1,Kt=Math.floor(rt*A)-1,Jt=Math.floor(rt*E)-1,At=Math.floor(rt*w)-1,Ct=Math.floor(rt*S)-1):console.log('Nie znam przyczyny :P')):R>=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y<=Wa&&k>=Fa&&M>=Ba?(rt=Va/R,(Mt=Ba/M)>(Bt=Fa/k)&&rt>Bt?(oa=Math.floor(Bt*g)-1,ra=Math.floor(Bt*T)-1,ft=Math.floor(Bt*f)-1,pt=Math.floor(Bt*_)-1,qt=Math.floor(Bt*b)-1,Kt=Math.floor(Bt*A)-1,Jt=Math.floor(Bt*E)-1,At=Math.floor(Bt*w)-1,
Ct=Math.floor(Bt*S)-1):Bt>Mt&&rt>Mt?(oa=Math.floor(Mt*g)-1,ra=Math.floor(Mt*T)-1,ft=Math.floor(Mt*f)-1,pt=Math.floor(Mt*_)-1,qt=Math.floor(Mt*b)-1,Kt=Math.floor(Mt*A)-1,Jt=Math.floor(Mt*E)-1,At=Math.floor(Mt*w)-1,Ct=Math.floor(Mt*S)-1):Bt>rt&&Mt>rt?(oa=Math.floor(rt*g)-1,ra=Math.floor(rt*T)-1,ft=Math.floor(rt*f)-1,pt=Math.floor(rt*_)-1,qt=Math.floor(rt*b)-1,Kt=Math.floor(rt*A)-1,Jt=Math.floor(rt*E)-1,At=Math.floor(rt*w)-1,Ct=Math.floor(rt*S)-1):console.log('Nie znam przyczyny :P')):R<=Va&&(e>=h||p>=h||u>=h||t>=h||n>=h||o>=h||r>=h||c>=h||d>=h)&&y>=Wa&&k>=Fa&&M>=Ba&&(Mt=Ba/M,(St=Wa/y)>(Bt=Fa/k)&&Mt>Bt?(oa=Math.floor(Bt*g)-1,ra=Math.floor(Bt*T)-1,ft=Math.floor(Bt*f)-1,pt=Math.floor(Bt*_)-1,qt=Math.floor(Bt*b)-1,Kt=Math.floor(Bt*A)-1,Jt=Math.floor(Bt*E)-1,At=Math.floor(Bt*w)-1,Ct=Math.floor(Bt*S)-1):Bt>St&&Mt>St?(oa=Math.floor(St*g)-1,ra=Math.floor(St*T)-1,ft=Math.floor(St*f)-1,pt=Math.floor(St*_)-1,qt=Math.floor(St*b)-1,Kt=Math.floor(St*A)-1,Jt=Math.floor(St*E)-1,At=Math.floor(St*w)-1,
Ct=Math.floor(St*S)-1):Bt>Mt&&St>Mt?(oa=Math.floor(Mt*g)-1,ra=Math.floor(Mt*T)-1,ft=Math.floor(Mt*f)-1,pt=Math.floor(Mt*_)-1,qt=Math.floor(Mt*b)-1,Kt=Math.floor(Mt*A)-1,Jt=Math.floor(Mt*E)-1,At=Math.floor(Mt*w)-1,Ct=Math.floor(Mt*S)-1):console.log('Nie znam przyczyny :P')),setTimeout((function(){oa>=h?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'spear',amount:oa}),v=[lt='spear',oa],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,oa)):console.log('Niezrekrutowano pikinierów - mniej niż wskazane minimum lub zbyt niski poziom koszar')}),6e3),setTimeout((function(){za>=3&&(ra>=h?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'sword',amount:ra}),v=[lt='sword',ra],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,ra)):console.log('Niezrekrutowano mieczników - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),12e3),setTimeout((function(){za>=5&&(ft>=h?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'axe',amount:ft}),v=[lt='axe',ft],
m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,ft)):console.log('Niezrekrutowano toporników - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),18e3),setTimeout((function(){za>=9&&(pt>=h?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'archer',amount:pt}),v=[lt='archer',pt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,pt)):console.log('Niezrekrutowano łuczników - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),24e3),setTimeout((function(){za>=11&&(qt>=h?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'light_cavalry',amount:qt}),v=[lt='light_cavalry',qt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,qt)):console.log('Niezrekrutowano lk - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),3e4),setTimeout((function(){za>=13&&(Kt>=h?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'mounted_archer',amount:Kt}),v=[lt='mounted_archer',Kt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),
J(Ca,lt,Kt)):console.log('Niezrekrutowano łk - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),36e3),setTimeout((function(){za>=15&&(Jt>=h?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'ram',amount:Jt}),v=[lt='ram',Jt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,Jt)):console.log('Niezrekrutowano taranów - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),42e3),setTimeout((function(){za>=17&&(At>=h?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'catapult',amount:At}),v=[lt='catapult',At],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,At)):console.log('Niezrekrutowano katapult - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),48e3),setTimeout((function(){za>=21&&(Ct>=h?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'heavy_cavalry',amount:Ct}),v=[lt='heavy_cavalry',Ct],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,Ct)):console.log('Niezrekrutowano ck - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),54e3))
;var e,t,n,o,r,c,d,u,p,g,T,f,_,b,A,E,w,S,R,y,k,M}()}()}function sn(){wa=n>0?Ya.length>=2?1:1==Ya.length?.5:.25:0,function(){Ea=Aa.length>0?Aa.reduce((function(e,t){return e+t})):0;!function(){va=n>0?Ea>=936e5?0:Ea<936e5&&Ea>=864e5?.1:Ea<864e5&&Ea>=72e6?.25:Ea<72e6&&Ea>=576e5?.4:Ea<576e5&&Ea>=432e5?.55:Ea<432e5&&Ea>=288e5?.7:Ea<288e5&&Ea>=144e5?.85:Ea<144e5&&Ea>=72e5?.925:Ea<72e5&&Ea>=36e5?.9625:1:0;e=Oe-je,t=Le-Ke,a=ye-Qe,o=Re-Xe,r=Ie-Ze,c=Ne-Je,d=xe-$e,u=ke-et,p=Me-tt,g=Math.floor(Q*va*wa),T=Math.floor(X*va*wa),f=Math.floor(F*va*wa),_=Math.floor(W*va*wa),b=Math.floor(H*va*wa),h=Math.floor(j*va*wa),E=Math.floor(K*va*wa),w=Math.floor(q*va*wa),S=Math.floor(Y*va*wa),R=g*Fe[0]+T*Fe[1]+f*Fe[2]+_*Fe[3]+b*Fe[4]+h*Fe[5]+E*Fe[7]+w*Fe[8]+S*Fe[6],y=g*qe[0]+T*qe[1]+f*qe[2]+_*qe[3]+b*qe[4]+h*qe[5]+E*qe[7]+w*qe[8]+S*qe[6],k=g*Ye[0]+T*Ye[1]+f*Ye[2]+_*Ye[3]+b*Ye[4]+h*Ye[5]+E*Ye[7]+w*Ye[8]+S*Ye[6],M=g*He[0]+T*He[1]+f*He[2]+_*He[3]+b*He[4]+h*He[5]+E*He[7]+w*He[8]+S*He[6],console.log(R,y,k,M),
e<=0||p<=0||u<=0||t<=0||a<=0||o<=0||r<=0||c<=0||d<=0?console.log('Brak jednostek do rekrutacji'):(R<=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y<=Qa&&k<=Xa&&M<=ja?(ia=g,ca=T,_t=f,gt=_,Yt=b,Qt=h,$t=E,Et=w,zt=S):R>=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y<=Qa&&k<=Xa&&M<=ja?(Lt=Ka/R,ia=Math.floor(Lt*g)-1,ca=Math.floor(Lt*T)-1,_t=Math.floor(Lt*f)-1,gt=Math.floor(Lt*_)-1,Yt=Math.floor(Lt*b)-1,Qt=Math.floor(Lt*h)-1,$t=Math.floor(Lt*E)-1,Et=Math.floor(Lt*w)-1,zt=Math.floor(Lt*S)-1):R<=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y>=Qa&&k<=Xa&&M<=ja?(Lt=Qa/y,ia=Math.floor(Lt*g)-1,ca=Math.floor(Lt*T)-1,_t=Math.floor(Lt*f)-1,gt=Math.floor(Lt*_)-1,Yt=Math.floor(Lt*b)-1,Qt=Math.floor(Lt*h)-1,$t=Math.floor(Lt*E)-1,Et=Math.floor(Lt*w)-1,zt=Math.floor(Lt*S)-1):R<=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y<=Qa&&k>=Xa&&M<=ja?(Lt=Xa/k,ia=Math.floor(Lt*g)-1,ca=Math.floor(Lt*T)-1,_t=Math.floor(Lt*f)-1,gt=Math.floor(Lt*_)-1,
Yt=Math.floor(Lt*b)-1,Qt=Math.floor(Lt*h)-1,$t=Math.floor(Lt*E)-1,Et=Math.floor(Lt*w)-1,zt=Math.floor(Lt*S)-1):R<=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y<=Qa&&k<=Xa&&M>=ja?(Lt=ja/M,ia=Math.floor(Lt*g)-1,ca=Math.floor(Lt*T)-1,_t=Math.floor(Lt*f)-1,gt=Math.floor(Lt*_)-1,Yt=Math.floor(Lt*b)-1,Qt=Math.floor(Lt*h)-1,$t=Math.floor(Lt*E)-1,Et=Math.floor(Lt*w)-1,zt=Math.floor(Lt*S)-1):R>=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y>=Qa&&k<=Xa&&M<=ja?(Rt=Qa/y)>(ct=Ka/R)?(ia=Math.floor(ct*g)-1,ca=Math.floor(ct*T)-1,_t=Math.floor(ct*f)-1,gt=Math.floor(ct*_)-1,Yt=Math.floor(ct*b)-1,Qt=Math.floor(ct*h)-1,$t=Math.floor(ct*E)-1,Et=Math.floor(ct*w)-1,zt=Math.floor(ct*S)-1):(ia=Math.floor(Rt*g)-1,ca=Math.floor(Rt*T)-1,_t=Math.floor(Rt*f)-1,gt=Math.floor(Rt*_)-1,Yt=Math.floor(Rt*b)-1,Qt=Math.floor(Rt*h)-1,$t=Math.floor(Rt*E)-1,Et=Math.floor(Rt*w)-1,
zt=Math.floor(Rt*S)-1):R<=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y>=Qa&&k>=Xa&&M<=ja?(Rt=Qa/y)>(Vt=Xa/k)?(ia=Math.floor(Vt*g)-1,ca=Math.floor(Vt*T)-1,_t=Math.floor(Vt*f)-1,gt=Math.floor(Vt*_)-1,Yt=Math.floor(Vt*b)-1,Qt=Math.floor(Vt*h)-1,$t=Math.floor(Vt*E)-1,Et=Math.floor(Vt*w)-1,zt=Math.floor(Vt*S)-1):(ia=Math.floor(Rt*g)-1,ca=Math.floor(Rt*T)-1,_t=Math.floor(Rt*f)-1,gt=Math.floor(Rt*_)-1,Yt=Math.floor(Rt*b)-1,Qt=Math.floor(Rt*h)-1,$t=Math.floor(Rt*E)-1,Et=Math.floor(Rt*w)-1,zt=Math.floor(Rt*S)-1):R<=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y>=Qa&&k<=Xa&&M>=ja?(Rt=Qa/y)>(It=ja/M)?(ia=Math.floor(It*g)-1,ca=Math.floor(It*T)-1,_t=Math.floor(It*f)-1,gt=Math.floor(It*_)-1,Yt=Math.floor(It*b)-1,Qt=Math.floor(It*h)-1,$t=Math.floor(It*E)-1,Et=Math.floor(It*w)-1,zt=Math.floor(It*S)-1):(ia=Math.floor(Rt*g)-1,ca=Math.floor(Rt*T)-1,_t=Math.floor(Rt*f)-1,gt=Math.floor(Rt*_)-1,Yt=Math.floor(Rt*b)-1,Qt=Math.floor(Rt*h)-1,$t=Math.floor(Rt*E)-1,
Et=Math.floor(Rt*w)-1,zt=Math.floor(Rt*S)-1):R>=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y<=Qa&&k>=Xa&&M<=ja?(Vt=Xa/k)>(ct=Ka/R)?(ia=Math.floor(ct*g)-1,ca=Math.floor(ct*T)-1,_t=Math.floor(ct*f)-1,gt=Math.floor(ct*_)-1,Yt=Math.floor(ct*b)-1,Qt=Math.floor(ct*h)-1,$t=Math.floor(ct*E)-1,Et=Math.floor(ct*w)-1,zt=Math.floor(ct*S)-1):(ia=Math.floor(Vt*g)-1,ca=Math.floor(Vt*T)-1,_t=Math.floor(Vt*f)-1,gt=Math.floor(Vt*_)-1,Yt=Math.floor(Vt*b)-1,Qt=Math.floor(Vt*h)-1,$t=Math.floor(Vt*E)-1,Et=Math.floor(Vt*w)-1,zt=Math.floor(Vt*S)-1):R<=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y<=Qa&&k>=Xa&&M>=ja?(Vt=Xa/k)>(It=ja/M)?(ia=Math.floor(It*g)-1,ca=Math.floor(It*T)-1,_t=Math.floor(It*f)-1,gt=Math.floor(It*_)-1,Yt=Math.floor(It*b)-1,Qt=Math.floor(It*h)-1,$t=Math.floor(It*E)-1,Et=Math.floor(It*w)-1,zt=Math.floor(It*S)-1):(ia=Math.floor(Vt*g)-1,ca=Math.floor(Vt*T)-1,_t=Math.floor(Vt*f)-1,gt=Math.floor(Vt*_)-1,Yt=Math.floor(Vt*b)-1,Qt=Math.floor(Vt*h)-1,
$t=Math.floor(Vt*E)-1,Et=Math.floor(Vt*w)-1,zt=Math.floor(Vt*S)-1):R>=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y<=Qa&&k<=Xa&&M>=ja?(It=ja/M)>(ct=Ka/R)?(ia=Math.floor(ct*g)-1,ca=Math.floor(ct*T)-1,_t=Math.floor(ct*f)-1,gt=Math.floor(ct*_)-1,Yt=Math.floor(ct*b)-1,Qt=Math.floor(ct*h)-1,$t=Math.floor(ct*E)-1,Et=Math.floor(ct*w)-1,zt=Math.floor(ct*S)-1):(ia=Math.floor(It*g)-1,ca=Math.floor(It*T)-1,_t=Math.floor(It*f)-1,gt=Math.floor(It*_)-1,Yt=Math.floor(It*b)-1,Qt=Math.floor(It*h)-1,$t=Math.floor(It*E)-1,Et=Math.floor(It*w)-1,zt=Math.floor(It*S)-1):R>=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y>=Qa&&k>=Xa&&M<=ja?(ct=Ka/R,(Rt=Qa/y)>(Vt=Xa/k)&&ct>Vt?(ia=Math.floor(Vt*g)-1,ca=Math.floor(Vt*T)-1,_t=Math.floor(Vt*f)-1,gt=Math.floor(Vt*_)-1,Yt=Math.floor(Vt*b)-1,Qt=Math.floor(Vt*h)-1,$t=Math.floor(Vt*E)-1,Et=Math.floor(Vt*w)-1,zt=Math.floor(Vt*S)-1):Vt>Rt&&ct>Rt?(ia=Math.floor(Rt*g)-1,ca=Math.floor(Rt*T)-1,_t=Math.floor(Rt*f)-1,gt=Math.floor(Rt*_)-1,
Yt=Math.floor(Rt*b)-1,Qt=Math.floor(Rt*h)-1,$t=Math.floor(Rt*E)-1,Et=Math.floor(Rt*w)-1,zt=Math.floor(Rt*S)-1):Vt>ct&&Rt>ct?(ia=Math.floor(ct*g)-1,ca=Math.floor(ct*T)-1,_t=Math.floor(ct*f)-1,gt=Math.floor(ct*_)-1,Yt=Math.floor(ct*b)-1,Qt=Math.floor(ct*h)-1,$t=Math.floor(ct*E)-1,Et=Math.floor(ct*w)-1,zt=Math.floor(ct*S)-1):console.log('Nie znam przyczyny :P')):R>=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y>=Qa&&k<=Xa&&M>=ja?(ct=Ka/R,(Rt=Qa/y)>(It=ja/M)&&ct>It?(ia=Math.floor(It*g)-1,ca=Math.floor(It*T)-1,_t=Math.floor(It*f)-1,gt=Math.floor(It*_)-1,Yt=Math.floor(It*b)-1,Qt=Math.floor(It*h)-1,$t=Math.floor(It*E)-1,Et=Math.floor(It*w)-1,zt=Math.floor(It*S)-1):It>Rt&&ct>Rt?(ia=Math.floor(Rt*g)-1,ca=Math.floor(Rt*T)-1,_t=Math.floor(Rt*f)-1,gt=Math.floor(Rt*_)-1,Yt=Math.floor(Rt*b)-1,Qt=Math.floor(Rt*h)-1,$t=Math.floor(Rt*E)-1,Et=Math.floor(Rt*w)-1,zt=Math.floor(Rt*S)-1):It>ct&&Rt>ct?(ia=Math.floor(ct*g)-1,ca=Math.floor(ct*T)-1,_t=Math.floor(ct*f)-1,gt=Math.floor(ct*_)-1,
Yt=Math.floor(ct*b)-1,Qt=Math.floor(ct*h)-1,$t=Math.floor(ct*E)-1,Et=Math.floor(ct*w)-1,zt=Math.floor(ct*S)-1):console.log('Nie znam przyczyny :P')):R>=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y<=Qa&&k>=Xa&&M>=ja?(ct=Ka/R,(It=ja/M)>(Vt=Xa/k)&&ct>Vt?(ia=Math.floor(Vt*g)-1,ca=Math.floor(Vt*T)-1,_t=Math.floor(Vt*f)-1,gt=Math.floor(Vt*_)-1,Yt=Math.floor(Vt*b)-1,Qt=Math.floor(Vt*h)-1,$t=Math.floor(Vt*E)-1,Et=Math.floor(Vt*w)-1,zt=Math.floor(Vt*S)-1):Vt>It&&ct>It?(ia=Math.floor(It*g)-1,ca=Math.floor(It*T)-1,_t=Math.floor(It*f)-1,gt=Math.floor(It*_)-1,Yt=Math.floor(It*b)-1,Qt=Math.floor(It*h)-1,$t=Math.floor(It*E)-1,Et=Math.floor(It*w)-1,zt=Math.floor(It*S)-1):Vt>ct&&It>ct?(ia=Math.floor(ct*g)-1,ca=Math.floor(ct*T)-1,_t=Math.floor(ct*f)-1,gt=Math.floor(ct*_)-1,Yt=Math.floor(ct*b)-1,Qt=Math.floor(ct*h)-1,$t=Math.floor(ct*E)-1,Et=Math.floor(ct*w)-1,
zt=Math.floor(ct*S)-1):console.log('Nie znam przyczyny :P')):R<=Ka&&(e>=A||p>=A||u>=A||t>=A||a>=A||o>=A||r>=A||c>=A||d>=A)&&y>=Qa&&k>=Xa&&M>=ja&&(It=ja/M,(Rt=Qa/y)>(Vt=Xa/k)&&It>Vt?(ia=Math.floor(Vt*g)-1,ca=Math.floor(Vt*T)-1,_t=Math.floor(Vt*f)-1,gt=Math.floor(Vt*_)-1,Yt=Math.floor(Vt*b)-1,Qt=Math.floor(Vt*h)-1,$t=Math.floor(Vt*E)-1,Et=Math.floor(Vt*w)-1,zt=Math.floor(Vt*S)-1):Vt>Rt&&It>Rt?(ia=Math.floor(Rt*g)-1,ca=Math.floor(Rt*T)-1,_t=Math.floor(Rt*f)-1,gt=Math.floor(Rt*_)-1,Yt=Math.floor(Rt*b)-1,Qt=Math.floor(Rt*h)-1,$t=Math.floor(Rt*E)-1,Et=Math.floor(Rt*w)-1,zt=Math.floor(Rt*S)-1):Vt>It&&Rt>It?(ia=Math.floor(It*g)-1,ca=Math.floor(It*T)-1,_t=Math.floor(It*f)-1,gt=Math.floor(It*_)-1,Yt=Math.floor(It*b)-1,Qt=Math.floor(It*h)-1,$t=Math.floor(It*E)-1,Et=Math.floor(It*w)-1,zt=Math.floor(It*S)-1):console.log('Nie znam przyczyny :P')),setTimeout((function(){ia>=A?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'spear',amount:ia}),v=[lt='spear',ia],
m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,ia)):console.log('Niezrekrutowano pikinierów - mniej niż wskazane minimum lub zbyt niski poziom koszar')}),6e3),setTimeout((function(){qa>=3&&(ca>=A?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'sword',amount:ca}),v=[lt='sword',ca],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,ca)):console.log('Niezrekrutowano mieczników - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),12e3),setTimeout((function(){qa>=5&&(_t>=A?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'axe',amount:_t}),v=[lt='axe',_t],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,_t)):console.log('Niezrekrutowano toporników - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),18e3),setTimeout((function(){qa>=9&&(gt>=A?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'archer',amount:gt}),v=[lt='archer',gt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),
J(Ca,lt,gt)):console.log('Niezrekrutowano łuczników - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),24e3),setTimeout((function(){qa>=11&&(Yt>=A?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'light_cavalry',amount:Yt}),v=[lt='light_cavalry',Yt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,Yt)):console.log('Niezrekrutowano lk - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),3e4),setTimeout((function(){qa>=13&&(Qt>=A?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'mounted_archer',amount:Qt}),v=[lt='mounted_archer',Qt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,Qt)):console.log('Niezrekrutowano łk - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),36e3),setTimeout((function(){qa>=15&&($t>=A?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'ram',amount:$t}),v=[lt='ram',$t],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,$t)):console.log('Niezrekrutowano taranów - mniej niż wskazane minimum lub zbyt niski poziom koszar'))
}),42e3),setTimeout((function(){qa>=17&&(Et>=A?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'catapult',amount:Et}),v=[lt='catapult',Et],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,Et)):console.log('Niezrekrutowano katapult - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),48e3),setTimeout((function(){qa>=21&&(zt>=A?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'heavy_cavalry',amount:zt}),v=[lt='heavy_cavalry',zt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,zt)):console.log('Niezrekrutowano ck - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),54e3));var e,t,a,o,r,c,d,u,p,g,T,f,_,b,h,E,w,S,R,y,k,M}()}()}function rn(){ya=c>0?Ja.length>=2?1:1==Ja.length?.5:.25:0,function(){Ra=Sa.length>0?Sa.reduce((function(e,t){return e+t})):0;!function(){
ka=c>0?Ra>=936e5?0:Ra<936e5&&Ra>=864e5?.1:Ra<864e5&&Ra>=72e6?.25:Ra<72e6&&Ra>=576e5?.4:Ra<576e5&&Ra>=432e5?.55:Ra<432e5&&Ra>=288e5?.7:Ra<288e5&&Ra>=144e5?.85:Ra<144e5&&Ra>=72e5?.925:Ra<72e5&&Ra>=36e5?.9625:1:0;e=Ve-je,t=We-Ke,a=Ue-Qe,n=Pe-Xe,o=De-Ze,r=Ge-Je,d=Be-$e,u=Ce-et,p=ze-tt,g=Math.floor(ie*ka*ya),T=Math.floor(le*ka*ya),f=Math.floor($*ka*ya),_=Math.floor(Z*ka*ya),b=Math.floor(ae*ka*ya),h=Math.floor(ne*ka*ya),A=Math.floor(oe*ka*ya),w=Math.floor(ee*ka*ya),S=Math.floor(te*ka*ya),R=g*Fe[0]+T*Fe[1]+f*Fe[2]+_*Fe[3]+b*Fe[4]+h*Fe[5]+A*Fe[7]+w*Fe[8]+S*Fe[6],y=g*qe[0]+T*qe[1]+f*qe[2]+_*qe[3]+b*qe[4]+h*qe[5]+A*qe[7]+w*qe[8]+S*qe[6],k=g*Ye[0]+T*Ye[1]+f*Ye[2]+_*Ye[3]+b*Ye[4]+h*Ye[5]+A*Ye[7]+w*Ye[8]+S*Ye[6],M=g*He[0]+T*He[1]+f*He[2]+_*He[3]+b*He[4]+h*He[5]+A*He[7]+w*He[8]+S*He[6],console.log(R,y,k,M),e<=0||p<=0||u<=0||t<=0||a<=0||n<=0||o<=0||r<=0||d<=0?console.log('Brak jednostek do rekrutacji'):(R<=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y<=an&&k<=nn&&M<=en?(la=g,da=T,
bt=f,mt=_,Ht=b,Xt=h,ea=A,wt=w,Dt=S):R>=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y<=an&&k<=nn&&M<=en?(Pt=tn/R,la=Math.floor(Pt*g)-1,da=Math.floor(Pt*T)-1,bt=Math.floor(Pt*f)-1,mt=Math.floor(Pt*_)-1,Ht=Math.floor(Pt*b)-1,Xt=Math.floor(Pt*h)-1,ea=Math.floor(Pt*A)-1,wt=Math.floor(Pt*w)-1,Dt=Math.floor(Pt*S)-1):R<=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y>=an&&k<=nn&&M<=en?(Pt=an/y,la=Math.floor(Pt*g)-1,da=Math.floor(Pt*T)-1,bt=Math.floor(Pt*f)-1,mt=Math.floor(Pt*_)-1,Ht=Math.floor(Pt*b)-1,Xt=Math.floor(Pt*h)-1,ea=Math.floor(Pt*A)-1,wt=Math.floor(Pt*w)-1,Dt=Math.floor(Pt*S)-1):R<=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y<=an&&k>=nn&&M<=en?(Pt=nn/k,la=Math.floor(Pt*g)-1,da=Math.floor(Pt*T)-1,bt=Math.floor(Pt*f)-1,mt=Math.floor(Pt*_)-1,Ht=Math.floor(Pt*b)-1,Xt=Math.floor(Pt*h)-1,ea=Math.floor(Pt*A)-1,wt=Math.floor(Pt*w)-1,
Dt=Math.floor(Pt*S)-1):R<=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y<=an&&k<=nn&&M>=en?(Pt=en/M,la=Math.floor(Pt*g)-1,da=Math.floor(Pt*T)-1,bt=Math.floor(Pt*f)-1,mt=Math.floor(Pt*_)-1,Ht=Math.floor(Pt*b)-1,Xt=Math.floor(Pt*h)-1,ea=Math.floor(Pt*A)-1,wt=Math.floor(Pt*w)-1,Dt=Math.floor(Pt*S)-1):R>=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y>=an&&k<=nn&&M<=en?(yt=an/y)>(dt=tn/R)?(la=Math.floor(dt*g)-1,da=Math.floor(dt*T)-1,bt=Math.floor(dt*f)-1,mt=Math.floor(dt*_)-1,Ht=Math.floor(dt*b)-1,Xt=Math.floor(dt*h)-1,ea=Math.floor(dt*A)-1,wt=Math.floor(dt*w)-1,Dt=Math.floor(dt*S)-1):(la=Math.floor(yt*g)-1,da=Math.floor(yt*T)-1,bt=Math.floor(yt*f)-1,mt=Math.floor(yt*_)-1,Ht=Math.floor(yt*b)-1,Xt=Math.floor(yt*h)-1,ea=Math.floor(yt*A)-1,wt=Math.floor(yt*w)-1,Dt=Math.floor(yt*S)-1):R<=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y>=an&&k>=nn&&M<=en?(yt=an/y)>(Wt=nn/k)?(la=Math.floor(Wt*g)-1,da=Math.floor(Wt*T)-1,bt=Math.floor(Wt*f)-1,
mt=Math.floor(Wt*_)-1,Ht=Math.floor(Wt*b)-1,Xt=Math.floor(Wt*h)-1,ea=Math.floor(Wt*A)-1,wt=Math.floor(Wt*w)-1,Dt=Math.floor(Wt*S)-1):(la=Math.floor(yt*g)-1,da=Math.floor(yt*T)-1,bt=Math.floor(yt*f)-1,mt=Math.floor(yt*_)-1,Ht=Math.floor(yt*b)-1,Xt=Math.floor(yt*h)-1,ea=Math.floor(yt*A)-1,wt=Math.floor(yt*w)-1,Dt=Math.floor(yt*S)-1):R<=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y>=an&&k<=nn&&M>=en?(yt=an/y)>(Nt=en/M)?(la=Math.floor(Nt*g)-1,da=Math.floor(Nt*T)-1,bt=Math.floor(Nt*f)-1,mt=Math.floor(Nt*_)-1,Ht=Math.floor(Nt*b)-1,Xt=Math.floor(Nt*h)-1,ea=Math.floor(Nt*A)-1,wt=Math.floor(Nt*w)-1,Dt=Math.floor(Nt*S)-1):(la=Math.floor(yt*g)-1,da=Math.floor(yt*T)-1,bt=Math.floor(yt*f)-1,mt=Math.floor(yt*_)-1,Ht=Math.floor(yt*b)-1,Xt=Math.floor(yt*h)-1,ea=Math.floor(yt*A)-1,wt=Math.floor(yt*w)-1,Dt=Math.floor(yt*S)-1):R>=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y<=an&&k>=nn&&M<=en?(Wt=nn/k)>(dt=tn/R)?(la=Math.floor(dt*g)-1,da=Math.floor(dt*T)-1,
bt=Math.floor(dt*f)-1,mt=Math.floor(dt*_)-1,Ht=Math.floor(dt*b)-1,Xt=Math.floor(dt*h)-1,ea=Math.floor(dt*A)-1,wt=Math.floor(dt*w)-1,Dt=Math.floor(dt*S)-1):(la=Math.floor(Wt*g)-1,da=Math.floor(Wt*T)-1,bt=Math.floor(Wt*f)-1,mt=Math.floor(Wt*_)-1,Ht=Math.floor(Wt*b)-1,Xt=Math.floor(Wt*h)-1,ea=Math.floor(Wt*A)-1,wt=Math.floor(Wt*w)-1,Dt=Math.floor(Wt*S)-1):R<=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y<=an&&k>=nn&&M>=en?(Wt=nn/k)>(Nt=en/M)?(la=Math.floor(Nt*g)-1,da=Math.floor(Nt*T)-1,bt=Math.floor(Nt*f)-1,mt=Math.floor(Nt*_)-1,Ht=Math.floor(Nt*b)-1,Xt=Math.floor(Nt*h)-1,ea=Math.floor(Nt*A)-1,wt=Math.floor(Nt*w)-1,Dt=Math.floor(Nt*S)-1):(la=Math.floor(Wt*g)-1,da=Math.floor(Wt*T)-1,bt=Math.floor(Wt*f)-1,mt=Math.floor(Wt*_)-1,Ht=Math.floor(Wt*b)-1,Xt=Math.floor(Wt*h)-1,ea=Math.floor(Wt*A)-1,wt=Math.floor(Wt*w)-1,Dt=Math.floor(Wt*S)-1):R>=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y<=an&&k<=nn&&M>=en?(Nt=en/M)>(dt=tn/R)?(la=Math.floor(dt*g)-1,
da=Math.floor(dt*T)-1,bt=Math.floor(dt*f)-1,mt=Math.floor(dt*_)-1,Ht=Math.floor(dt*b)-1,Xt=Math.floor(dt*h)-1,ea=Math.floor(dt*A)-1,wt=Math.floor(dt*w)-1,Dt=Math.floor(dt*S)-1):(la=Math.floor(Nt*g)-1,da=Math.floor(Nt*T)-1,bt=Math.floor(Nt*f)-1,mt=Math.floor(Nt*_)-1,Ht=Math.floor(Nt*b)-1,Xt=Math.floor(Nt*h)-1,ea=Math.floor(Nt*A)-1,wt=Math.floor(Nt*w)-1,Dt=Math.floor(Nt*S)-1):R>=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y>=an&&k>=nn&&M<=en?(dt=tn/R,(yt=an/y)>(Wt=nn/k)&&dt>Wt?(la=Math.floor(Wt*g)-1,da=Math.floor(Wt*T)-1,bt=Math.floor(Wt*f)-1,mt=Math.floor(Wt*_)-1,Ht=Math.floor(Wt*b)-1,Xt=Math.floor(Wt*h)-1,ea=Math.floor(Wt*A)-1,wt=Math.floor(Wt*w)-1,Dt=Math.floor(Wt*S)-1):Wt>yt&&dt>yt?(la=Math.floor(yt*g)-1,da=Math.floor(yt*T)-1,bt=Math.floor(yt*f)-1,mt=Math.floor(yt*_)-1,Ht=Math.floor(yt*b)-1,Xt=Math.floor(yt*h)-1,ea=Math.floor(yt*A)-1,wt=Math.floor(yt*w)-1,Dt=Math.floor(yt*S)-1):Wt>dt&&yt>dt?(la=Math.floor(dt*g)-1,da=Math.floor(dt*T)-1,bt=Math.floor(dt*f)-1,
mt=Math.floor(dt*_)-1,Ht=Math.floor(dt*b)-1,Xt=Math.floor(dt*h)-1,ea=Math.floor(dt*A)-1,wt=Math.floor(dt*w)-1,Dt=Math.floor(dt*S)-1):console.log('Nie znam przyczyny :P')):R>=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y>=an&&k<=nn&&M>=en?(dt=tn/R,(yt=an/y)>(Nt=en/M)&&dt>Nt?(la=Math.floor(Nt*g)-1,da=Math.floor(Nt*T)-1,bt=Math.floor(Nt*f)-1,mt=Math.floor(Nt*_)-1,Ht=Math.floor(Nt*b)-1,Xt=Math.floor(Nt*h)-1,ea=Math.floor(Nt*A)-1,wt=Math.floor(Nt*w)-1,Dt=Math.floor(Nt*S)-1):Nt>yt&&dt>yt?(la=Math.floor(yt*g)-1,da=Math.floor(yt*T)-1,bt=Math.floor(yt*f)-1,mt=Math.floor(yt*_)-1,Ht=Math.floor(yt*b)-1,Xt=Math.floor(yt*h)-1,ea=Math.floor(yt*A)-1,wt=Math.floor(yt*w)-1,Dt=Math.floor(yt*S)-1):Nt>dt&&yt>dt?(la=Math.floor(dt*g)-1,da=Math.floor(dt*T)-1,bt=Math.floor(dt*f)-1,mt=Math.floor(dt*_)-1,Ht=Math.floor(dt*b)-1,Xt=Math.floor(dt*h)-1,ea=Math.floor(dt*A)-1,wt=Math.floor(dt*w)-1,
Dt=Math.floor(dt*S)-1):console.log('Nie znam przyczyny :P')):R>=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y<=an&&k>=nn&&M>=en?(dt=tn/R,(Nt=en/M)>(Wt=nn/k)&&dt>Wt?(la=Math.floor(Wt*g)-1,da=Math.floor(Wt*T)-1,bt=Math.floor(Wt*f)-1,mt=Math.floor(Wt*_)-1,Ht=Math.floor(Wt*b)-1,Xt=Math.floor(Wt*h)-1,ea=Math.floor(Wt*A)-1,wt=Math.floor(Wt*w)-1,Dt=Math.floor(Wt*S)-1):Wt>Nt&&dt>Nt?(la=Math.floor(Nt*g)-1,da=Math.floor(Nt*T)-1,bt=Math.floor(Nt*f)-1,mt=Math.floor(Nt*_)-1,Ht=Math.floor(Nt*b)-1,Xt=Math.floor(Nt*h)-1,ea=Math.floor(Nt*A)-1,wt=Math.floor(Nt*w)-1,Dt=Math.floor(Nt*S)-1):Wt>dt&&Nt>dt?(la=Math.floor(dt*g)-1,da=Math.floor(dt*T)-1,bt=Math.floor(dt*f)-1,mt=Math.floor(dt*_)-1,Ht=Math.floor(dt*b)-1,Xt=Math.floor(dt*h)-1,ea=Math.floor(dt*A)-1,wt=Math.floor(dt*w)-1,Dt=Math.floor(dt*S)-1):console.log('Nie znam przyczyny :P')):R<=tn&&(e>=E||p>=E||u>=E||t>=E||a>=E||n>=E||o>=E||r>=E||d>=E)&&y>=an&&k>=nn&&M>=en&&(Nt=en/M,(yt=an/y)>(Wt=nn/k)&&Nt>Wt?(la=Math.floor(Wt*g)-1,
da=Math.floor(Wt*T)-1,bt=Math.floor(Wt*f)-1,mt=Math.floor(Wt*_)-1,Ht=Math.floor(Wt*b)-1,Xt=Math.floor(Wt*h)-1,ea=Math.floor(Wt*A)-1,wt=Math.floor(Wt*w)-1,Dt=Math.floor(Wt*S)-1):Wt>yt&&Nt>yt?(la=Math.floor(yt*g)-1,da=Math.floor(yt*T)-1,bt=Math.floor(yt*f)-1,mt=Math.floor(yt*_)-1,Ht=Math.floor(yt*b)-1,Xt=Math.floor(yt*h)-1,ea=Math.floor(yt*A)-1,wt=Math.floor(yt*w)-1,Dt=Math.floor(yt*S)-1):Wt>Nt&&yt>Nt?(la=Math.floor(Nt*g)-1,da=Math.floor(Nt*T)-1,bt=Math.floor(Nt*f)-1,mt=Math.floor(Nt*_)-1,Ht=Math.floor(Nt*b)-1,Xt=Math.floor(Nt*h)-1,ea=Math.floor(Nt*A)-1,wt=Math.floor(Nt*w)-1,Dt=Math.floor(Nt*S)-1):console.log('Nie znam przyczyny :P')),setTimeout((function(){la>=E?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'spear',amount:la}),v=[lt='spear',la],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,la)):console.log('Niezrekrutowano pikinierów - mniej niż wskazane minimum lub zbyt niski poziom koszar')}),6e3),setTimeout((function(){Za>=3&&(da>=E?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,
unit_type:'sword',amount:da}),v=[lt='sword',da],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,da)):console.log('Niezrekrutowano mieczników - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),12e3),setTimeout((function(){Za>=5&&(bt>=E?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'axe',amount:bt}),v=[lt='axe',bt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,bt)):console.log('Niezrekrutowano toporników - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),18e3),setTimeout((function(){Za>=9&&(mt>=E?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'archer',amount:mt}),v=[lt='archer',mt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,mt)):console.log('Niezrekrutowano łuczników - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),24e3),setTimeout((function(){Za>=11&&(Ht>=E?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'light_cavalry',amount:Ht}),v=[lt='light_cavalry',Ht],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),
J(Ca,lt,Ht)):console.log('Niezrekrutowano lk - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),3e4),setTimeout((function(){Za>=13&&(Xt>=E?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'mounted_archer',amount:Xt}),v=[lt='mounted_archer',Xt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,Xt)):console.log('Niezrekrutowano łk - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),36e3),setTimeout((function(){Za>=15&&(ea>=E?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'ram',amount:ea}),v=[lt='ram',ea],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,ea)):console.log('Niezrekrutowano taranów - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),42e3),setTimeout((function(){Za>=17&&(wt>=E?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'catapult',amount:wt}),v=[lt='catapult',wt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,wt)):console.log('Niezrekrutowano katapult - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),48e3),
setTimeout((function(){Za>=21&&(Dt>=E?(i.emit(l.BARRACKS_RECRUIT,{village_id:Ca,unit_type:'heavy_cavalry',amount:Dt}),v=[lt='heavy_cavalry',Dt],m.trigger(s.RECRUIT_QUEUE_JOB_STARTED,v),J(Ca,lt,Dt)):console.log('Niezrekrutowano ck - mniej niż wskazane minimum lub zbyt niski poziom koszar'))}),54e3));var e,t,a,n,o,r,d,u,p,g,T,f,_,b,h,A,w,S,R,y,k,M}()}()}!function(){var e=o.getSelectedCharacter().getVillageList();console.log(e);var t=6e4*e.length,a=6e4*e.length+6e4*e.length,n=6e4*e.length+6e4*e.length+6e4*e.length;e.forEach((function(e,o){setTimeout((function(){d.forEach((function(t){if(e.data.villageId==t){console.log('wioska:'+e.data.villageId),Ca=e.getId(),je=e.unitInfo.units.spear.own+e.unitInfo.units.spear.recruiting,Ke=e.unitInfo.units.sword.own+e.unitInfo.units.sword.recruiting,Qe=e.unitInfo.units.axe.own+e.unitInfo.units.axe.recruiting,Xe=e.unitInfo.units.archer.own+e.unitInfo.units.archer.recruiting,Ze=e.unitInfo.units.light_cavalry.own+e.unitInfo.units.light_cavalry.recruiting,
Je=e.unitInfo.units.mounted_archer.own+e.unitInfo.units.mounted_archer.recruiting,$e=e.unitInfo.units.ram.own+e.unitInfo.units.ram.recruiting,et=e.unitInfo.units.catapult.own+e.unitInfo.units.catapult.recruiting,tt=e.unitInfo.units.heavy_cavalry.own+e.unitInfo.units.heavy_cavalry.recruiting,at=e.buildingData.data.barracks.level,nt=e.buildingQueue.data.queue,ot=e.getRecruitingQueues(),it=ot.barracks.jobs;var a=e.getResources().getComputed(),n=a.wood,o=a.clay,i=a.iron,l=a.food;Oa=l.currentStock,La=n.currentStock,Pa=o.currentStock,Ua=i.currentStock,it.forEach((function(e){ta=e.clientRecruitingTime,ua=1e3*(e.data.time_completed-e.data.start_time),aa=ua-ta,pa.push(aa)})),on()}}))}),6e4*o),setTimeout((function(){u.forEach((function(t){if(e.data.villageId==t){je=e.unitInfo.units.spear.own+e.unitInfo.units.spear.recruiting,Ke=e.unitInfo.units.sword.own+e.unitInfo.units.sword.recruiting,Qe=e.unitInfo.units.axe.own+e.unitInfo.units.axe.recruiting,
Xe=e.unitInfo.units.archer.own+e.unitInfo.units.archer.recruiting,Ze=e.unitInfo.units.light_cavalry.own+e.unitInfo.units.light_cavalry.recruiting,Je=e.unitInfo.units.mounted_archer.own+e.unitInfo.units.mounted_archer.recruiting,$e=e.unitInfo.units.ram.own+e.unitInfo.units.ram.recruiting,et=e.unitInfo.units.catapult.own+e.unitInfo.units.catapult.recruiting,tt=e.unitInfo.units.heavy_cavalry.own+e.unitInfo.units.heavy_cavalry.recruiting,za=e.buildingData.data.barracks.level,Da=e.buildingQueue.data.queue,ot=e.getRecruitingQueues(),Ga=ot.barracks.jobs;var a=e.getResources().getComputed(),n=a.wood,o=a.clay,i=a.iron,l=a.food;Ba=l.currentStock,Va=n.currentStock,Wa=o.currentStock,Fa=i.currentStock,Ga.forEach((function(e){ta=e.clientRecruitingTime,ua=1e3*(e.data.time_completed-e.data.start_time),aa=ua-ta,fa.push(aa)})),ln()}}))}),6e4*(o+1)+t),setTimeout((function(){g.forEach((function(t){if(e.data.villageId==t){je=e.unitInfo.units.spear.own+e.unitInfo.units.spear.recruiting,
Ke=e.unitInfo.units.sword.own+e.unitInfo.units.sword.recruiting,Qe=e.unitInfo.units.axe.own+e.unitInfo.units.axe.recruiting,Xe=e.unitInfo.units.archer.own+e.unitInfo.units.archer.recruiting,Ze=e.unitInfo.units.light_cavalry.own+e.unitInfo.units.light_cavalry.recruiting,Je=e.unitInfo.units.mounted_archer.own+e.unitInfo.units.mounted_archer.recruiting,$e=e.unitInfo.units.ram.own+e.unitInfo.units.ram.recruiting,et=e.unitInfo.units.catapult.own+e.unitInfo.units.catapult.recruiting,tt=e.unitInfo.units.heavy_cavalry.own+e.unitInfo.units.heavy_cavalry.recruiting,qa=e.buildingData.data.barracks.level,Ya=e.buildingQueue.data.queue,ot=e.getRecruitingQueues(),Ha=ot.barracks.jobs;var a=e.getResources().getComputed(),n=a.wood,o=a.clay,i=a.iron,l=a.food;ja=l.currentStock,Ka=n.currentStock,Qa=o.currentStock,Xa=i.currentStock,Ha.forEach((function(e){ta=e.clientRecruitingTime,ua=1e3*(e.data.time_completed-e.data.start_time),aa=ua-ta,Aa.push(aa)})),sn()}}))}),6e4*(o+2)+a),setTimeout((function(){
T.forEach((function(t){if(e.data.villageId==t){je=e.unitInfo.units.spear.own+e.unitInfo.units.spear.recruiting,Ke=e.unitInfo.units.sword.own+e.unitInfo.units.sword.recruiting,Qe=e.unitInfo.units.axe.own+e.unitInfo.units.axe.recruiting,Xe=e.unitInfo.units.archer.own+e.unitInfo.units.archer.recruiting,Ze=e.unitInfo.units.light_cavalry.own+e.unitInfo.units.light_cavalry.recruiting,Je=e.unitInfo.units.mounted_archer.own+e.unitInfo.units.mounted_archer.recruiting,$e=e.unitInfo.units.ram.own+e.unitInfo.units.ram.recruiting,et=e.unitInfo.units.catapult.own+e.unitInfo.units.catapult.recruiting,tt=e.unitInfo.units.heavy_cavalry.own+e.unitInfo.units.heavy_cavalry.recruiting,Za=e.buildingData.data.barracks.level,Ja=e.buildingQueue.data.queue,ot=e.getRecruitingQueues(),$a=ot.barracks.jobs;var a=e.getResources().getComputed(),n=a.wood,o=a.clay,i=a.iron,l=a.food;en=l.currentStock,tn=n.currentStock,an=o.currentStock,nn=i.currentStock,$a.forEach((function(e){ta=e.clientRecruitingTime,
ua=1e3*(e.data.time_completed-e.data.start_time),aa=ua-ta,Sa.push(aa)})),rn()}}))}),6e4*(o+3)+n)}))}(),m.trigger(s.RECRUIT_QUEUE_CYCLE_END)},$.getLogs=function(){return h},$.clearLogs=function(){return h=[],T.set(Q,h),m.trigger(s.RECRUIT_QUEUE_CLEAR_LOGS),h},$.start=function(){E=!0,m.trigger(s.RECRUIT_QUEUE_START)},$.stop=function(e=n.USER_STOP){E=!1,m.trigger(s.RECRUIT_QUEUE_STOP,{reason:e}),m.trigger(s.RECRUIT_QUEUE_CYCLE_END,e),e===n.USER_STOP&&J(p.RECRUIT_STOP,'stop','')},$.getSettings=function(){return _},$.isInitialized=function(){return A},$.isRunning=function(){return E},$.getNextCycleDate=function(){return w},$})),define('two/recruitQueue/events',[],(function(){angular.extend(s,{RECRUIT_QUEUE_JOB_STARTED:'recruit_queue_job_started',RECRUIT_QUEUE_START:'recruit_queue_start',RECRUIT_QUEUE_STOP:'recruit_queue_stop',RECRUIT_QUEUE_CLEAR_LOGS:'recruit_queue_clear_logs',RECRUIT_QUEUE_CYCLE_BEGIN:'recruit_queue_cycle_begin',RECRUIT_QUEUE_CYCLE_END:'recruit_queue_cycle_end'})})),
