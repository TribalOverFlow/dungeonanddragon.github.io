...
;P=m>=10&&m<20?1.05:m>=20&&m<25?1.15:m>=25?1.3:1,function(){const ne=h[t.TROOPS_TRAINING];var ie,le,oe,se;console.log(ne),z=1==b?1.01:2==b?1.02:3==b?1.03:4==b?1.04:5==b?1.05:6==b?1.06:7==b?1.07:8==b?1.08:9==b?1.09:10==b?1.1:1,M=h[t.TROOPS_DOMINATION]?1.1:1,O=p>0?k[6]+' minut':u>0?k[5]+' minut':r>0||c>0?k[4]+' minuty':a>0?k[3]+' minut':e>0||n>0||i>0||d>0?k[2]+' minut':s>0?k[1]+' minut':l>0||o>0||g>0?k[0]+' minut':k[7]+' minut',L=m>=7&&m<14?x[1]:m>=14&&m<23?x[2]:m>=23?x[3]:x[0],U=N/(M*ne),C=U/P,G=C/z,D=N/z,B='deff'==T?G*(e*A[0]+a*A[1]+i*A[3]+s*A[6])+D*(p*A[11])+C*(n*A[2]+l*A[4]+o*A[5]+r*A[7]+c*A[8])+d*A[12]+g*A[9]+u*A[10]:'off'==T?C*(e*A[0]+a*A[1]+i*A[3]+s*A[6])+p*A[11]+G*(n*A[2]+l*A[4]+o*A[5]+r*A[7]+c*A[8])+D*(d*A[12])+g*A[9]+u*A[10]:C*(e*A[0]+a*A[1]+i*A[3]+s*A[6])+p*A[11]+C*(n*A[2]+l*A[4]+o*A[5]+r*A[7]+c*A[8])+d*A[12]+g*A[9]+u*A[10],V=e*E[0]+a*E[1]+n*E[2]+i*E[3]+l*E[4]+o*E[5]+s*E[6]+r*E[7]+c*E[8]+g*E[9]+u*E[10]+p*E[11]+d*E[12],
...
t.register(s.BUILDER_QUEUE_BUILDING_SEQUENCES_REMOVED,K.buildingSequenceRemoved),t.register(s.BUILDER_QUEUE_SETTINGS_CHANGE,K.saveSettings),t.register(s.BUILDER_QUEUE_START,K.started),t.register(s.BUILDER_QUEUE_STOP,K.stopped),c.getScreenWithInjectedScope('!twoverflow_builder_queue_window',v),M=v.closeWindow,v.closeWindow=H,v.$watch('settings[SETTINGS.ACTIVE_SEQUENCE].value',(function(e,t){e!==t&&K.generateBuildingSequences()})),v.$watch('editorView.selectedSequence.value',(function(e,t){G?G=!1:e!==t&&(U?j((function(){K.generateBuildingSequencesEditor(),U=!1}),(function(){v.editorView.selectedSequence={name:t,value:t},G=!0})):K.generateBuildingSequencesEditor())}))};return function(){R=i.getGameData().getBuildings(),L.generateBuildingsLevelPoints(),y=n.getSettings(),S=e.addMenuButton2('Budowniczy',20,g('i18n')('general.description',a.loc.ale,'builder_queue')),S.addEventListener('click',Q),h.register(s.BUILDER_QUEUE_START,(function(){x=!0,S.classList.remove('btn-orange'),
...
A.villagesLabel=I,A.villagesInfo=k,A.exceptionVillages=t.getExceptionVillages(),A.exceptionLogs=t.getExceptionLogs(),A.logs=t.getLogs(),A.visibleLogs=[],A.showCycleTimer=!1,A.nextCycleCountdown=0,A.saveButtonColor='orange',A.settingsMap=v.settingsMap,A.pagination={count:A.logs.length,offset:0,loader:L,limit:m.getPaginationLimit()},v.injectScope(A),D(),B(),L(),P(),function(){let e=t.getNextCycleDate();e&&(A.showCycleTimer=!0,A.nextCycleCountdown=e-h.gameTime(),x=setInterval((function(){A.nextCycleCountdown-=1e3}),1e3))}(),A.switchFarm=M,A.selectTab=z,A.saveSettings=C,A.clearLogs=t.clearLogs,A.jumpToVillage=u.jumpToVillage,A.openVillageInfo=r.openVillageInfo,A.showReport=b.showReport,A.removeIgnored=U,A.removeIncluded=G;let n=new _('twoverflow_farm_overflow_window',(function(){clearInterval(x)}));n.register(s.ARMY_PRESET_UPDATE,D,!0),n.register(s.ARMY_PRESET_DELETED,D,!0),n.register(s.GROUPS_UPDATED,B,!0),n.register(s.GROUPS_CREATED,B,!0),n.register(s.GROUPS_DESTROYED,B,!0),
...
f.push(e[t])})),c.forEach((function(t){w.push(e[t])})),d.forEach((function(t){h.push(e[t])}))},Q=function(){A=[],v=[],S=[],R=[],y=[],k=[],I=[],x=[],O=[],L=[],N=[],P=[],M=[],z=[],C=[],U=[],G=[],D=[],B=[],V=[],W=[],q=[],Y=[],F=[];const e=i.getGroupList().getGroups(),a=p[t.GROUP1],n=p[t.GROUP2],l=p[t.GROUP3],o=p[t.GROUP4],s=p[t.GROUP5],r=p[t.GROUP6],c=p[t.GROUP7],d=p[t.GROUP8],u=p[t.GROUP9],g=p[t.GROUP10],m=p[t.GROUP11],b=p[t.GROUP12],T=p[t.GROUP13],E=p[t.GROUP14],_=p[t.GROUP15],f=p[t.GROUP16],w=p[t.GROUP17],h=p[t.GROUP18],j=p[t.GROUP19],H=p[t.GROUP20],K=p[t.GROUP21],Q=p[t.GROUP22],X=p[t.GROUP23],Z=p[t.GROUP24];a.forEach((function(t){A.push(e[t])})),n.forEach((function(t){v.push(e[t])})),l.forEach((function(t){S.push(e[t])})),o.forEach((function(t){R.push(e[t])})),s.forEach((function(t){y.push(e[t])})),r.forEach((function(t){k.push(e[t])})),c.forEach((function(t){I.push(e[t])})),d.forEach((function(t){x.push(e[t])})),u.forEach((function(t){O.push(e[t])})),g.forEach((function(t){
