...
village_id:e.getId(),location:_.MASS_SCREEN,premium:!1},a)},q=function(e,t){const a=e.getBuildingData().getDataForBuilding(t);if(a.upgradeability===g.POSSIBLE){const t=a.nextLevelCosts,i=e.getResources().getComputed();if(i.clay.currentStock-N[r.PRESERVE_CLAY]<t.clay||i.iron.currentStock-N[r.PRESERVE_IRON]<t.iron||i.wood.currentStock-N[r.PRESERVE_WOOD]<t.wood)return g.NOT_ENOUGH_RESOURCES}return a.upgradeability},j=function(e){for(let t in e)if(e[t]<w[t])return!1;return!0},W=function(e,t){let a={time:T.gameTime(),villageId:e,building:t.building,level:t.level};return x.unshift(a),x.length>500&&x.splice(x.length-500,x.length),E.set(C,x),!0};let Y={};return Y.start=function(){if(!P)return b.trigger(s.BUILDER_QUEUE_NO_SEQUENCES),!1;R=!0,f=setInterval(z,6e4),h=setInterval(G,6e3),e((function(){U().forEach((function(e){const t=L.getVillage(e);t.isInitialized()||u.initializeVillage(t)})),z(),G()}),['all_villages_ready']),b.trigger(s.BUILDER_QUEUE_START)},Y.stop=function(){R=!1,clearInterval(f),
...
COMMAND_QUEUE_ADD_INVALID_OFFICER:'command_queue_add_invalid_officer',COMMAND_QUEUE_ADD_RELOCATE_DISABLED:'command_queue_add_relocate_disabled'})})),define('two/builderQueue/ui',['two/ui','two/builderQueue','two/utils','two/ready','two/Settings','two/builderQueue/settings','two/builderQueue/settings/map','two/builderQueue/sequenceStatus','conf/buildingTypes','two/EventScope','queues/EventQueue','helper/time'],(function(e,i,d,u,p,_,b,T,f,h,w,v){let S,A,y,R,I=n.getGroupList(),k={},L=!1,x={sequencesAvail:!0,modal:{}},O={sequencesAvail:!0},N={};const P={SETTINGS:'settings',SEQUENCES:'sequences',LOGS:'logs'};let C,M={},z={},G=!1,U=!1;const D=function(e,t,a){return a?parseInt(Math.round(e*Math.pow(t,a-1)),10):0},B=function(e,a,i){if(i>=e.length){let a=i-e.length+1;for(;a--;)e.push(t)}e.splice(i,0,e.splice(a,1)[0])},V=function(e,t){switch(i.addBuildingSequence(e,t)){case T.SEQUENCE_SAVED:return!0;case T.SEQUENCE_EXISTS:
...
c.getModal('!twoverflow_builder_queue_name_sequence_modal',e)},j=function(e){S.selectedTab=e},W=function(){R.setAll(R.decode(S.settings)),G=!1},Y=function(){i.isRunning()?i.stop():i.start()},F=function(e,t){let i=a.$new();i.title=g('i18n')('discard_changes_title',a.loc.ale,'builder_queue'),i.text=g('i18n')('discard_changes_text',a.loc.ale,'builder_queue'),i.submitText=g('i18n')('discard',a.loc.ale,'common'),i.cancelText=g('i18n')('cancel',a.loc.ale,'common'),i.switchColors=!0,i.submit=function(){i.closeWindow(),e&&e()},i.cancel=function(){i.closeWindow(),t&&t()},c.getModal('modal_attention',i)},H=function(){G?F((function(){C()})):C()},K={updateGroups:function(){S.groups=p.encodeList(I.getGroups(),{type:'groups',disabled:!0})},updateSequences:function(){const e=R.get(_.BUILDING_SEQUENCES);S.sequences=p.encodeList(e,{type:'keys',disabled:!1})},generateBuildingSequences:function(){O.generateSequences()},generateBuildingSequencesEditor:function(){x.generateBuildingSequence()},
...
barbarian:!a.character_id,officers:!0,effects:!0}),b=d.getTravelTimeForDistance(g,_,m,v.TYPES.ATTACK);if(l.some(e=>{const t=e.getCommandListModel().getOutgoingCommands(!0,!0).filter(e=>e.targetVillageId===a.id&&'forward'===e.data.direction);if(t.length)return t.some(e=>Math.abs(o+b-e.time_completed)<n)}))return t(i.BUSY_TARGET);e()}),C=()=>new Promise((e,t)=>{if(m)return e();o.emit(l.MAP_GET_VILLAGE_DETAILS,{my_village_id:this.villageId,village_id:a.id,num_reports:0},a=>{if(!this.running)return t(i.USER_STOP);const n=a.commands.own.filter(e=>e.type===v.TYPES.ATTACK&&'forward'===e.direction),o=n.some(e=>e.start_village_id!==this.villageId),l=n.some(e=>e.start_village_id===this.villageId);if(Te(l,o,!0))return t(i.BUSY_TARGET);e()})});let M=new Promise((e,t)=>{b().then(T).then(A).then(R).then(I).then(L).then(O).then(N).then(P).then(C).then(e).catch(t)}),G=new Promise((e,t)=>{F=setTimeout(()=>{this.running&&t(i.EXPIRED_STEP)},3e4)});Promise.race([M,G]).then(()=>{if(!this.running)return!1
...
v.exceptionVillages.included.forEach((function(e){N(e)})),v.exceptionVillages.ignored.forEach((function(e){N(e)}))},C=function(){t.isRunning()?t.stop():t.start()},M=function(e){v.selectedTab=e},z=function(){S.setAll(S.decode(v.settings)),v.saveButtonColor='orange',f.notif('success',g('i18n')('settings_saved',a.loc.ale,'farm_overflow'))},G=function(e){const t=S.get(b.GROUP_IGNORE);if(!n.getGroupList().getGroupVillageIds(t).includes(e))return!1;o.emit(l.GROUPS_UNLINK_VILLAGE,{group_id:t,village_id:e})},U=function(e){S.get(b.GROUP_INCLUDE).forEach((function(t){n.getGroupList().getGroupVillageIds(t).includes(e)&&o.emit(l.GROUPS_UNLINK_VILLAGE,{group_id:t,village_id:e})}))},D=function(){v.presets=E.encodeList(y.getPresets(),{disabled:!1,type:'presets'})},B=function(){v.groups=E.encodeList(R.getGroups(),{disabled:!1,type:'groups'}),v.groupsWithDisabled=E.encodeList(R.getGroups(),{disabled:!0,type:'groups'})},V=function(){v.running=!0,
...
console.log('spyMaster window closed')}));e.register(s.SPY_MASTER_START,v),e.register(s.SPY_MASTER_STOP,S),c.getScreenWithInjectedScope('!twoverflow_spy_master_window',_)};return function(){b=t.getSettings(),E=e.addMenuButton('Zwiadowca',170),E.addEventListener('click',A),
